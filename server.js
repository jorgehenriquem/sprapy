...
function consoleLogWithStyle(message, colorCode = 37) {
    console.log(`\x1b[${colorCode}m%s\x1b[0m`, `${formatConsoleDate(new Date())} ${message}`);
}
...
async function randomScroll(page) {
    await page.evaluate(async () => {
        ...
        if (elemento) {
            ...
            for (let i = 0; i < 5; i++) { // Reduzido para 5 iterações
                ...
                await new Promise(resolve => setTimeout(resolve, randomBetween(200, 500))); // Tempo de espera reduzido
            }
        }
    });
}
...
(async () => {
  ...
  while (true) {
    consoleLogWithStyle(`Likes: ${contadorLikes}, Deslikes: ${contadorDeslikes}`);
    await sleep(1000);
    let selectorVisible = await isSelectorVisible(page, 'span.Typs\\(display-1-strong\\)');
    ...
    if (selectorVisible) {
        ...
        consoleLogWithStyle("Seletor ainda não encontrado. Continuando o loop...", '31');
        continue; // Pula para a próxima iteração do loop
    }
}

async function isSelectorVisible(page, selector) {
    return page.waitForSelector(selector, { visible: true, timeout: 5000 })
